...
homicide_sum <- homicide_sum[-c(51:51),] 

 ##Plot the data with horizontal error bars                                        
plot_homicides <- homicide_sum %>%
  ggplot(aes(x=(estimate), y= reorder(city_name, +estimate), color="gray100", fill="grey100")) +
  geom_point()+
  ...
  geom_errorbarh(aes(xmax= conf.high, xmin=conf.low, height=0, color="grey100"))+
  theme_dark()+
